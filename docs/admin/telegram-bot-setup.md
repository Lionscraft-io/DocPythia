...
This guide explains how to set up and configure the Telegram bot integration for the DocPythia multi-stream scanner system.

## Overview

The Telegram bot adapter allows DocPythia to ingest messages from Telegram channels and groups in real-time. Messages are processed through the same batch analysis pipeline as other message sources (CSV, Zulipchat), with conversation grouping, RAG retrieval, and documentation proposal generation.
...
1. Node.js project with DocPythia installed
