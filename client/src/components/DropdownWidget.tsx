    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
          <div className={`w-full h-full max-w-full max-h-full relative ${theme === 'dark' ? 'bg-gray-900' : 'bg-white'}`}>
            {/* Header */}
            ...
            <div className="relative w-full h-[calc(100vh-80px)]">
              {isLoading && (
                <div className={`absolute inset-0 flex items-center justify-center ${theme === 'dark' ? 'bg-gray-900' : 'bg-white'}`}>
                  <div className="flex items-center space-x-3">
                    <div className={`animate-spin rounded-full h-8 w-8 border-b-2 ${theme === 'dark' ? 'border-white' : 'border-gray-900'}`}></div>
                    <span className={`text-lg ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Loading Chat...</span>
                  </div>
                </div>
              )}
              <iframe
