      ...
      {sections.map((section) => (
        <div key={section.id} id={section.id}>
          {section.type && section.type !== "text" ? (
            ...
            <>
              {section.level === 1 && (
                <h1 className="text-3xl font-bold tracking-tight" data-testid={`heading-${section.id}`}>
                  {section.title}
                </h1>
              )}
              {section.level === 2 && (
                <h2 className="text-2xl font-semibold tracking-tight" data-testid={`heading-${section.id}`}>
                  {section.title}
                </h2>
              )}
              {section.level === 3 && (
