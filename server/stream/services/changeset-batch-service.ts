...
export class ChangesetBatchService {
  ...
  async generatePR(
    ...
    const branchName = `neardocs-updates-${batch.batchId}`;
  ...
  private generateCommitMessage(batch: ChangesetBatch, appliedCount: number): string {
    return `docs: Apply ${appliedCount} documentation updates from NearDocsAI

Batch ID: ${batch.batchId}
Total proposals: ${batch.totalProposals}
Successfully applied: ${appliedCount}

Generated by NearDocsAI automated documentation system`;
  }
  ...
  private generatePRBody(userBody: string, appliedCount: number, failedCount: number): string {
    ...
    if (failedCount > 0) {
      ...
      body += `\n⚠️ **Warning:** Some proposals could not be applied. Review the changeset history for details.\n`;
    }

    body += `\nGenerated by [NearDocsAI](https://github.com/lionscraft/NearDocsAI)`;
